...
- Organize documentation by **what the documentation is about**, not by the history of the blog posts that produced it.
- Legacy blog posts are **source material, not the destination structure**.
- Decompose old posts and redistribute their useful content into the correct buckets.
...
- Never create a folder or page merely because a legacy blog post exists.
- A single legacy post may contribute to many buckets.
- Multiple legacy posts may contribute to one bucket or page.
- Historical context such as “failed attempt,” “old,” or “current” should normally be represented with page titles, tags, or a Lessons Learned/Gotchas section—not by creating a bucket whose only purpose is to mirror the blog's history.
---
...
Whenever a new document or migration is added, it may:
...
A logical bucket represents a cross-cutting system, policy, or convention that isn't a concrete physical item and isn't a single installed software/OS/service — e.g. network addressing schemes, subnetting plans, naming conventions, or other shared logical structure that spans multiple hardware and software entries.
...
Some entries can be confusing to know where to put. An example would be a bash script that automatically tweak some features when the battery is disconnected. While the bash script might make this fall under the software bucket, it acually belongs in the hardware bucket. The reason why is that the script's end goal is to improve the physical's battery usage. 
...
If the answer to any of those questions involes/relates to a physical item, then it goes into the hardware bucket. 
...
- Top-level domains are allowed to describe broad contexts such as:
  ...
  - `Homelab/`
  - `Blog/`
- These top-level domains are **organizational contexts, not content buckets**. Their child buckets must still follow the Item/OS/Service rule in §2.
...
- Do not mix unrelated systems merely because they appeared in the same blog post.
...
- If the source material says an AI tool was used for a specific piece of content, code, or decision, keep that attribution in the migrated page, including which specific AI model/tool it was (e.g. "generated with Google Gemini", "a regex written with Claude's help") — do not silently drop AI-authorship notes during migration.
...
When converting a legacy blog post:

1. Read the entire post.
2. Identify the concrete items, OSes, services, and tools involved.
3. Extract durable technical topics from the post's headings and content.
4. Assign each topic to the appropriate Item/OS/Service bucket.
5. Split one old post across multiple buckets when necessary.
6. Merge material from multiple old posts when they document the same system.
7. Create a sequence folder when the material is genuinely step-by-step.
8. Remove obsolete, redundant, or purely narrative material unless it has useful historical/diagnostic value.
9. Cross-link related buckets instead of duplicating content.
10. Update the migration map so the source of migrated material remains traceable.
11. Apply §15 (Image Curation) to every image in the post: keep only images that diagnose a problem, verify a fix, record a non-reproducible fact, or document a genuine constraint; discard narration, rejected-option comparisons, cosmetic before/afters, duplicates, and off-topic borrowed images. Rename and move every kept image into the domain's shared image folder.
12. Do not carry over a legacy post's numbered citation/reference list. Where a cited source is genuinely useful to a reader (e.g. authoritative documentation), fold it in inline as a normal link or as a glossary reference (§6) — otherwise drop it. Preserving the blog's citation numbering or a "References" section is not required.

---

## 15. Image Curation

Legacy posts often carry far more images than a wiki entry needs — process narration, theme comparisons, before/after polish shots, and borrowed illustrations. Images are content like any other: they get migrated selectively, not wholesale (§1).

### Keep an image when it does one of the following

- **Diagnoses a problem.** It shows a real error, broken state, or failure mode the reader could otherwise hit — a stack trace, a broken render, a wrong UI state. Pair it with the fix per §12 (Gotchas/Lessons Learned).
...
- **Was borrowed from unrelated subject matter.** An image pulled from a different legacy post purely to illustrate a general point (e.g. "here's some other screenshot to show what a caption looks like") does not belong to this bucket's subject and should be dropped even if the point it illustrates is kept in prose.
...
- All kept images for a domain go in one shared folder at the domain's root, e.g. `Blog/Images/`, not scattered per-bucket. Buckets reference the shared pool rather than each maintaining their own.
...
> "Was this image in the original post?"
...
- Captalize the aberration and name of prodects. 
...
- A file that was manually created or edited by the user (a script, a container
  definition, a config file, etc.) must NOT be pasted into a wiki page as a
  code block. Reference it via a GitHub link instead.
- Exception: a code block is allowed only when either is true:
  - it exists purely to show the reader what a file's contents look like
    (illustrative, not the maintained copy), or
  - the file is temporary/throwaway and isn't something that gets maintained.
- Decision rule: "Was this file manually created/edited by the user?"
  If yes → link, don't paste (unless one of the two exceptions above applies).
- If the GitHub URL is not known yet, put a placeholder link in the page:
  `[filename](TODO)`.
- Do not guess missing file names, file paths, URLs, relationships, or other
  context. If the source material does not provide enough information to
  confidently determine what is needed, ask the user a clarification question.
- All unresolved migration clarification questions are stored in
  `Migration Q&A.json` at the vault root.
...
- `type` identifies the reason clarification is required. Useful values include
  `missing-context`, `missing-file`, `missing-url`, `ambiguous-reference`,
  `missing-history`, and `missing-relationship`.
- `question` is the direct question the user needs to answer.
- `context` briefly explains why the question was generated and what source
  material caused the uncertainty.
- `related` lists the migration page or pages affected by the answer.
- `answer` must initially be `null`. Since only the user answers these
  questions, no additional status field is required.
- When the user answers a question, incorporate the answer into the migration
  and remove that question object from `Migration Q&A.json`.
- If `Migration Q&A.json` contains no questions, delete the file entirely.
- If a new clarification question is needed and `Migration Q&A.json` does not
  exist, create it using the structure above.

## 18. Migration Clarification Questions

- Migration should never guess when the available source material is
  insufficient to confidently determine a file, bucket, relationship,
  procedure, or other important context.
- Continue migrating everything that can be determined confidently, while
  recording unresolved questions for the user in `Migration Q&A.json`.
...
- Questions about missing context from other vault entries are temporary
  migration questions. They are not permanent buckets, pages, or categories.
- Once the user answers a question, use the answer to complete or correct the
  affected migration material and remove the answered question from
  `Migration Q&A.json`.
- If no questions remain, delete `Migration Q&A.json`. Its absence means there
  are no pending migration questions.

## 19. Flagged Concerns 
- Migration must never silently fix, silently ignore, or silently work around something that looks like a security problem or an unintended
- mistake in the source material (plaintext secrets, disabled certificate verification, overly permissive allow-lists, hard-coded credentials, etc.).
- When migration surfaces something like this, add a flagged-concern question to Migration Q&A.json asking the user to confirm it
- was intentional — even when the migration can otherwise proceed with full confidence and nothing else is ambiguous.
- This is additive to §18: a flagged-concern question does not block migrating the surrounding material, it just needs an explicit answer
- before the page is treated as final/publishable.
- Once the user confirms (fix it / leave it / already handled elsewhere), incorporate the answer and remove the question per the normal §17 lifecycle.
...
- [ ] Am I accidentally recreating the structure of a legacy blog post?
...
Before finalizing the vault:

- [ ] Every top-level domain has exactly one `Index.md`.
...
- [ ] Legacy posts have been treated as source material rather than the site structure.
...
- [ ] Any unresolved migration ambiguity has a targeted question in `Migration Q&A.json`, and the file is deleted when no questions remain.
